...
namespace Unity.Labs.MARS.Providers
{
    static class ARKitFaceLandmarksExtensions
    {
        ...
        internal static void GenerateLandmarks(this XRSDKFace xrsdkFace)
        {
            ...
            {
                var point = PointFromTriangle(xrsdkFace.Mesh.vertices, xrsdkFace.Mesh.triangles, landmarkTriangleIndices[i]);
                k_LandmarkPositions[i] = point;
            }

            MapToAuthorLandmarks(k_LandmarkPositions, xrsdkFace);
        }
        ...
        static void MapToAuthorLandmarks(Vector3[] vertexPositions, XRSDKFace face)
        {
