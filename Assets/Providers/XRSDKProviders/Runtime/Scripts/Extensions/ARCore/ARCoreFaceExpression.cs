...
namespace Unity.Labs.MARS.Providers
{
    ...
    class ARCoreLeftEyeCloseExpression : ARCoreFaceExpression
    {
        ...
        public override float GetCoefficient(Vector3[] landmarks)
        {
            var leftEyeTop = landmarks[(int)ARCoreFaceMeshLandmark.LeftEyeTop];
            var leftEyeBottom = landmarks[(int)ARCoreFaceMeshLandmark.LeftEyeBottom];

            return CoefficientUtils.FromInverseDistance(leftEyeBottom, leftEyeTop, maxDistance, minDistance);
        }
    }
    ...
    class ARCoreRightEyeCloseExpression : ARCoreFaceExpression
    {
        ...
        public override float GetCoefficient(Vector3[] landmarks)
        {
            var rightEyeTop = landmarks[(int)ARCoreFaceMeshLandmark.RightEyeTop];
            var rightEyeBottom = landmarks[(int)ARCoreFaceMeshLandmark.RightEyeBottom];

            return CoefficientUtils.FromInverseDistance(rightEyeBottom, rightEyeTop, maxDistance, minDistance);
        }
    }
