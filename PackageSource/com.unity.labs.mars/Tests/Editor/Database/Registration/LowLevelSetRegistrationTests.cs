...
namespace Unity.Labs.MARS.Data.Tests.QueryData
{
    ...
    {
        ParallelSetData m_Data;

        [OneTimeSetUp]
        public void Setup()
        {
            const int initialCapacity = 8;
            m_Data = new ParallelSetData(initialCapacity);
        }
        ...
        public void Clear()
        {
            ...
            {
                GameObject go;
                TestUtils.CreateDefaultTestContext(out go);
        ...
        public void ResizeOnInsertIfNecessary()
        {
            ...
            for (var i = 0; i < queryCount; i++)
            {
                TestUtils.CreateDefaultTestContext(out GameObject go);
                RegisterFromSetArgs(GetTestQueryArgs(go));
            }
