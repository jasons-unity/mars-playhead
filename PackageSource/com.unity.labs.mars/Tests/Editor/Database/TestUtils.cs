...
using Unity.Labs.MARS.Data.Tests;
...
namespace Unity.Labs.MARS.Tests
{
    static class TestUtils
    {
        ...
        public static Proxy CreateDefaultTestContext(out GameObject go, int index = 0)
        {
            go = new GameObject("context test object " + index);
            k_ToDestroy.Add(go);
        ...
        public static void DestroyDefaultTestContexts()
        {
        ...
        public static SetChildArgs DefaultSetChildArgs(Proxy context)
        {
            return new SetChildArgs
            {
                tryBestMatchArgs = new TryBestMatchArguments(new Conditions(context), context.exclusivity),
                required = true,
        ...
        public static Relations GetRelations(GameObject gameObject)
        {
            ...
            foreach(var context in gameObject.GetComponentsInChildren<Proxy>())
            {
                var memberArgs = DefaultSetChildArgs(context);
                objectToArgs.Add(context, memberArgs);
            }
