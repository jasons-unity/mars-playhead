...
using Unity.Labs.MARS.Behaviors;
...
namespace Unity.Labs.MARS
{
    ...
    /// </summary>
    [AddComponentMenu("")]
    class MARSSession : MonoBehaviour, IUsesFunctionalityInjection
    {
        ...
        static readonly Type[] k_SessionComponents = { typeof(MARSSession) };
...
#pragma warning restore 649
...
#if UNITY_EDITOR
        /// <summary>
        /// Optional build settings for this scene
        /// </summary>
        [SerializeField]
        BuildPreset m_BuildSettings;
#endif
        ...
        public MARSCamera cameraReference { get { return m_CameraReference; } }

        public static MARSSession Instance { get; private set; }
        internal static bool TestMode { private get; set; }

        public FunctionalityIsland island { get { return m_Island; } }
        public Capabilities requirements { get { return m_Requirements; } }

#if UNITY_EDITOR
        public BuildPreset buildSettings { get { return m_BuildSettings; } }

        public MarsMarkerLibrary MarkerLibrary { get { return m_MarkerLibrary; } }
#endif
        ...
        public static void EnsureRuntimeState(Transform overrideUserRef = null)
        {
            ...
            if (MARSCore.instance.BlockEnsureSession)
                return;
            ...
            using (var undoBlock = new UndoBlock("Ensure MARS Runtime State"))
            {
                if (Instance == null)
                {
                    ...
                    var sessionObject = new GameObject(k_ObjectName, k_SessionComponents);
                    undoBlock.RegisterCreatedObject(sessionObject);
                    Instance = sessionObject.GetComponent<MARSSession>();
                    var sessionTransform = sessionObject.transform;
        ...
        static bool SessionConfigured(MARSSession session)
        {
            ...
            return sessionTransform.parent == null && sessionTransform.GetSiblingIndex() == 0 && sessionCamera != null &&
                sessionCamera.transform.parent == sessionTransform && session.m_UserReference != null &&
                sessionObject.name == k_ObjectName && sessionObject.activeInHierarchy;
        }
        ...
        static void EnsureSessionConfigured(MARSSession session, UndoBlock undoBlock, Transform overrideUserRef = null)
        {
            ...
            if (!TestMode && session.m_CameraReference.transform.parent != sessionTransform)
            {
                LogRuntimeIssue("MARSCamera was not parented to the MARSSession object.  Re-parenting.");
                MatchSessionTransformWithCameraParent(sessionTransform);
        ...
        public static MARSSession EnsureSessionInActiveScene()
        {
            ...
            using (var undoBlock = new UndoBlock("Ensure Session in Active Scene", TestMode))
            {
                if (!session)
                {
                    var sessionObject = new GameObject(k_ObjectName, k_SessionComponents);
                    undoBlock.RegisterCreatedObject(sessionObject);
                    session = sessionObject.GetComponent<MARSSession>();
                    var sessionTransform = sessionObject.transform;
                    sessionTransform.SetAsFirstSibling();
        ...
        public void CheckCapabilities()
        {
            ...
            var modified = false;
            var isFaceScene = false;
            var scene = SceneManager.GetActiveScene();
            var rootGameObjects = scene.GetRootGameObjects();
            foreach (var gameObject in rootGameObjects)
            {
                foreach (var requirer in gameObject.GetComponentsInChildren<IRequiresTraits>(true))
                {
                    ...
                    foreach (var requirement in requirer.GetRequiredTraits())
                    {
                        ...
                        if (!string.IsNullOrEmpty(requirement.TraitName))
                            gatheredTraitRequirements.Add(requirement);
                    }
                }

                isFaceScene |= gameObject.GetComponentInChildren<IUsesFaceTracking>(true) != null;
            }
            ...
            // Store scene info
            isFaceScene |= gatheredTraitRequirements.Contains(TraitDefinitions.Face);

            var marsCore = MARSCore.instance;
            // any face scene without any previous functionality island set
            if (m_Island == null && isFaceScene)
            {
                modified = true;
                m_Island = marsCore.defaultFaceIsland;
            }

            // any face scene with no established build settings yet
            if (m_BuildSettings == null && isFaceScene)
            {
                modified = true;
                m_BuildSettings = marsCore.defaultFaceBuildSettings;
            }
