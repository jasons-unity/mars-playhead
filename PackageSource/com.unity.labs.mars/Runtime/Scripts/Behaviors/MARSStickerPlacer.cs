...
namespace Unity.Labs.MARS.Behaviors
{
    public class MARSStickerPlacer : MonoBehaviour, IUsesPlaneFinding, IUsesMRHitTesting, IUsesCameraOffset
    {
        class SpawnedObject
        {
            ...
            public void OnComplete(GameObject newObject)
            {
                // If load took too long, this object might already be destroyed
                if (!obj)
                {
                    Destroy(newObject);
                    return;
...
#pragma warning restore 649
                ...
                if (Physics.Raycast(ray, out hit, Mathf.Infinity, m_RaycastLayers))
                {
                    ...
                    if ((m_ObjectLayer & hitLayer) != 0)
                    {
                        ...
                        if (m_Selected == null)
                        {
                            ...
                            m_LastValid = PoseFromHit(mainCamera.transform, hit);
                    ...
                    {
                        m_Selected = SpawnAsset(PoseFromHit(mainCamera.transform, hit));
        ...
        GameObject SpawnAsset(Pose pose)
        {
            ...
            var asset = m_Assets[Random.Range(0, m_Assets.Length)];
            var obj = asset.Instantiate(spawnedObject.OnComplete);
