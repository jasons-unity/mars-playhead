...
namespace Unity.Labs.MARS
{
    ...
    public abstract class PlaneVoxelGrid<TLayerPlane> where TLayerPlane : LayerPlaneData
    {
            ...
            VoxelGridOrientation orientation, Bounds containedBounds, float voxelSize, VoxelPlaneFindingParams planeFindingParams)
        {
            ...
            m_Voxels = new PlaneVoxel[m_Layers * m_LayerSize];
            for (var i = 0; i < m_Layers; ++i)
            {
                for (var j = 0; j < m_Columns; ++j)
                {
                    ...
                    {
                        m_Voxels[VoxelIndex(i, j, k)] = new PlaneVoxel { LayerCoordinates = new Vector2Int(j, k) };
                    }
                }
            }
        ...
        int VoxelIndex(int layer, int column, int row) { return layer * m_LayerSize + column * m_Rows + row; }

        protected PlaneVoxel GetVoxel(int layer, int column, int row) { return m_Voxels[VoxelIndex(layer, column, row)]; }
        ...
        {
            var index = VoxelIndex(layer, column, row);
            var voxel = m_Voxels[index];
