...
namespace Unity.Labs.MARS
{
    ...
    public class CameraFPSModeHandler
    {
        ...
        public void HandleGUIInput(Rect rect, Event currentEvent, EventType type)
        {
        ...
        {
            // Calculate Rotation from `CameraFlyModeContext.cs` in trunk
            rotation = Quaternion.AngleAxis(invertY ? -m_MouseDelta.y : m_MouseDelta.y * .003f * Mathf.Rad2Deg,
                rotation * Vector3.right) * rotation;
            rotation = Quaternion.AngleAxis(m_MouseDelta.x * .003f * Mathf.Rad2Deg, Vector3.up) * rotation;
            return rotation;
        }
    }
}
