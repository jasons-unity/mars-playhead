...
namespace Unity.Labs.MARS.Data
{
    ...
    public partial class MARSDatabase : IModuleMarsUpdate, IUsesCameraOffset, IUsesCameraPose
    {
        ...
        /// </summary>
        internal readonly Dictionary<Type, FillRequirementTypeAction> TypeToRequirementFill =
            new Dictionary<Type, FillRequirementTypeAction>();

        /// <summary>
        /// Maps each trait type to the function that will update the value of a trait requirement of that type.
        /// Filled by generated code.
        /// </summary>
        internal readonly Dictionary<Type, UpdateRequirementTypeAction> TypeToRequirementUpdate =
            new Dictionary<Type, UpdateRequirementTypeAction>();
        ...
        internal void CacheTraitReferences(List<int> indexes, List<int> filteredIndexes, List<Conditions> conditions, ref List<CachedTraitCollection> references)
        {
        ...
        internal void MarkDataUsedAction(List<int> indices, List<int> dataId, List<QueryMatchID> queryMatchId, ref List<Exclusivity> exclusivity)
        {
            foreach (var i in indices)
            {
                MarkDataUsedForUpdates(dataId[i], queryMatchId[i], exclusivity[i]);
            }
        }

        internal void ReserveDataForQueryMatch(int dataID, QueryMatchID queryMatchID, Exclusivity exclusivity)
        {
            Dictionary<QueryMatchID, bool> dirtyStates;
            if (m_QueryDataDirtyStates.TryGetValue(dataID, out dirtyStates))
                dirtyStates[queryMatchID] = false;
            else
            {
                dirtyStates = Pools.QueryDataDirtyStates.Get();
                dirtyStates.Add(queryMatchID, false);
        ...
        public void UnmarkDataUsedForUpdates(QueryMatchID queryMatchID)
        {
            ...
            var queryID = queryMatchID.queryID;
            var dataUsedByQuery = DataUsedByQueries[queryID];
        ...
        void UnreserveDataForQueryMatch(int dataID, QueryMatchID queryMatchID)
        {
            ...
            else
            {
                ...
                if (ReservedData.TryGetValue(dataID, out dataOwner) && queryMatchID.Equals(dataOwner))
        ...
        {
            int dataID;
            return DataUsedByQueryMatches.TryGetValue(queryMatchID, out dataID) && QueryDataDirty(dataID, queryMatchID);
        }
        ...
        bool QueryDataDirty(int dataID, QueryMatchID queryMatchID)
        {
            ...
            bool dirty;
            if (m_QueryDataDirtyStates.TryGetValue(dataID, out dirtyStates) &&
                dirtyStates.TryGetValue(queryMatchID, out dirty))
            {
                dirtyStates[queryMatchID] = false;
                return dirty;
            }
        ...
        public bool TryUpdateQueryMatchData(int dataID, Conditions conditions, ContextTraitRequirements requirements, QueryResult result)
        {
        ...
        void SetDirtyIfNeeded(int dataID)
        {
            ...
            if (m_QueryDataDirtyStates.TryGetValue(dataID, out dirtyStates))
            {
                k_QueryMatchIDs.Clear();
                foreach (var kvp in dirtyStates)
                {
                    k_QueryMatchIDs.Add(kvp.Key);
                }

                foreach (var queryMatchID in k_QueryMatchIDs)
                {
                    dirtyStates[queryMatchID] = true;
                }
            }
        }
        ...
        internal void FillQueryResultRequirements(int dataId, ContextTraitRequirements requirements, QueryResult result)
        {
            foreach (var requirement in requirements)
            {
                ...
                TypeToRequirementFill[requirement.Type](requirement, dataId, result);
            }
        }

        internal bool TryFillTraitRequirementsUpdate(int dataId, ContextTraitRequirements requirements, QueryResult result)
        {
            foreach (var requirement in requirements)
            {
                GetTraitProvider(out MARSTraitDataProvider<Pose> poseTraitProvider);
                ...
                if (!TypeToRequirementUpdate[requirement.Type](requirement, dataId, result))
                    return false;
            }
