...
namespace Unity.Labs.MARS.Query
{
        ...
        List<int>>
    {
        ...
        internal void FindBestStage(List<int> workingIndices, List<int> verifiedMatchIndices,
            List<Dictionary<int, float>> ratingsById,
            List<Exclusivity> exclusivities,
            ref List<int> bestMatchIds)
        {
            ...
            ResolveConflicts(requestMap, ratingsById);
        ...
        /// </returns>
        internal Dictionary<int, int> ResolveConflicts(Dictionary<int, DataIdRequestMap> indicesRequestingId, List<Dictionary<int, float>> ratings)
        {
            m_IdRemovalBuffer.Clear();
            ...
            SolveReservedLoop(indicesRequestingId, ratings);
        ...
        /// </summary>
        void SolveReservedLoop(Dictionary<int, DataIdRequestMap> indicesRequestingId, List<Dictionary<int, float>> ratings)
        {
            // don't allow more iterations than we have reserved queries, as a safeguard against infinite loop
            var safetyCounter = 0;
            var counterMax = ratings.Count;
            int conflictCount;
            do
            {
                m_GlobalReservedConflictSet.Clear();
                foreach (var kvp in indicesRequestingId)
                {
                    ...
                    SolveReserved(kvp.Key, kvp.Value, ratings);
                }
        ...
        /// </summary>
        void SolveReserved(int dataId, DataIdRequestMap map, List<Dictionary<int, float>> ratings)
        {
            ...
            switch (reservedIndexes.Count)
            {
                ...
                case 1:
                    ...
                    break;
                // as int as more than 1 Reserved query is asking for this ID, there is a conflict.
                default:
                    // out of all the reserved queries requesting this id, which has the best score ?
                    var bestIndex = BestScoringIndexForId(dataId, reservedIndexes, ratings);
                    if (bestIndex == -1)
                        return;
        ...
        static int BestScoringIndexForId<T>(int dataId, T indices, List<Dictionary<int, float>> ratings)
            where T : class, IEnumerable<int>
        {
            var bestIndex = (int)ReservedDataIDs.Invalid;
            var bestScore = -1f;
        ...
        void SolveShared(List<Dictionary<int, float>> ratings, HashSet<int> sharedIndexes)
        {
        ...
        internal Dictionary<int, DataIdRequestMap> BuildRequestMap(List<int> workingIndices,
            List<Dictionary<int, float>> ratings, List<Exclusivity> exclusivities)
        {
