...
{
    class IncompleteGroupFilterTransform : DataTransform<List<int>, int[][]>
    {
        static readonly List<int> k_ClearBuffer = new List<int>();

        public IncompleteGroupFilterTransform() { Process = FilterIncompleteGroups; }

        static void FilterIncompleteGroups(List<int> workingMemberIndices, List<int> workingGroupIndices, ref int[][] allGroupMemberIndices)
        {
            k_ClearBuffer.Clear();
            ...
            foreach (var invalidatedGroupMemberIndex in k_ClearBuffer)
            {
                workingGroupIndices.Remove(invalidatedGroupMemberIndex);
            }
        }
    }
