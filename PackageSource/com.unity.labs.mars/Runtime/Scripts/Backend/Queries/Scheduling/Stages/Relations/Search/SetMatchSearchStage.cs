...
namespace Unity.Labs.MARS.Query
{
    ...
    class SetMatchSearchTransform : DataTransform<List<int>,
        int[][],
        RelationDataPair[][],
        RelationRatingsData[],
        float[],
        SetSearchData[],
        List<RelationMembership[]>,
        List<Exclusivity>,
        List<Dictionary<int, float>>,
        List<int>>
    {
        ...
        List<KeyValuePair<int, float>> m_OrderPairs = new List<KeyValuePair<int, float>>(k_InitialCollectionCapacity);
        ...
        internal SetRatingConfiguration RatingConfiguration { get; set; }

        public SetMatchSearchTransform()
        {
        ...
        internal static void CalculateSolveOrder(float[] weights, List<int> workingIndices,
            List<KeyValuePair<int, float>> setIndexToWeight)
        {
            setIndexToWeight.Clear();
            foreach (var i in workingIndices)
            {
                setIndexToWeight.Add(new KeyValuePair<int, float>(i, weights[i]));
            }

            setIndexToWeight.Sort(k_Comparer);
        }
        ...
        void ProcessStage(List<int> workingIndices, List<int> outputIndices,
            ...
            SetSearchData[] allSetSearchData,
            List<RelationMembership[]> allRelationMemberships,
            List<Exclusivity> allMemberExclusivities,
            List<Dictionary<int, float>> allMemberRatings,
            ref List<int> allMemberAssignments)
        {
            ...
            if (m_TickCount == 0)
            {
                ...
                // Determine the order we are going to answer Sets
                CalculateSolveOrder(allSetOrderWeights, workingIndices, m_OrderPairs);

                var setCount = m_OrderPairs.Count;
                m_SetsPerTick = Mathf.Clamp(Mathf.CeilToInt(setCount / (float)FrameBudget), 1, setCount);
            }

            // if we need to force completion,
            var endIndex = NeedsCompletion ? m_OrderPairs.Count
                : Mathf.Clamp(m_SolveIndex + m_SetsPerTick, m_SolveIndex, m_OrderPairs.Count);

            for (var solveIndex = m_SolveIndex; solveIndex < endIndex; solveIndex++)
            {
                var kvp = m_OrderPairs[solveIndex];
                ...
                var searchData = allSetSearchData[setIndex];
                ...
                var searchSpaceSize = GetSetSearchSpaceSize(memberIndices, allMemberRatings);
                ...
                */
                ...
                for (var i = begin; i < begin + matchBuffer.SetSize; i++)
                {
                ...
                if (solveIndex >= m_OrderPairs.Count - 1)
                    continue;

                for (var i = solveIndex + 1; i < m_OrderPairs.Count; i++)
                {
                    var otherSetIndex = m_OrderPairs[i].Key;
            ...
            if (endIndex == m_OrderPairs.Count)
            {
        ...
        internal bool TryCombinationsWithoutRelations(SetSearchData searchData, PreviousSetMatches previousMatches)
        {
            var done = false;
            ...
            else
            {
                do
                {
                    ...
                    if (AnyDuplicates(hypothesis) || previousMatches.AssignmentPreviouslyUsed(hypothesis))
                    {
                        duplicateCount++;
                        continue;
                    }
        ...
        internal static bool AnyDuplicates(KeyValuePair<int, float>[] hypothesis)
        {
        ...
        internal static int GetSetSearchSpaceSize(int[] memberIndices, List<Dictionary<int, float>> allMemberRatings)
        {
            var size = 1;
    ...
    class SetMatchSearchStage : QueryStage<SetMatchSearchTransform>
    {
        public SetMatchSearchStage(SetMatchSearchTransform transformation)
