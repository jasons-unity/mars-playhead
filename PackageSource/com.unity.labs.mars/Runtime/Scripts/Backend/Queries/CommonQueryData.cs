...
namespace Unity.Labs.MARS.Query
{
    ...
    public struct CommonQueryData
    {
        ...
        {
            const string str = "Re-acquire on Loss: {0}, Timeout: {1}, Match Interval: {2}";

            return string.Format(str, reacquireOnLoss, overrideTimeout ? "default" : timeOut.ToString(), updateMatchInterval);
        }
    }
}
