...
namespace Unity.Labs.MARS.Query
{
    partial class FindMatchProposalsTransform
    {
        internal static void GetStartingIdSet(ConditionRatingsData data, HashSet<int> idSet)
        {
            ...
            var vector3Ratings = data[typeof(UnityEngine.Vector3)];
            if (vector3Ratings != null && vector3Ratings.Count > 0)
            {
                foreach (var kvp in vector3Ratings[0])
                {
                    idSet.Add(kvp.Key);
                }
        ...
        static bool FindIntersection(ConditionRatingsData data, HashSet<int> matchSet)
        {
            ...
            var vector3Data = data[typeof(UnityEngine.Vector3)];
            if (vector3Data != null)
            {
                foreach (var dictionary in vector3Data)
                {
                    k_IDsMatchingCondition.Clear();
                    foreach (var kvp in dictionary)
                    {
