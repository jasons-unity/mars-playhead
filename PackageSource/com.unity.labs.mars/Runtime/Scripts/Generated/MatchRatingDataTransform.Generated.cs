...
namespace Unity.Labs.MARS.Query
{
    partial class MatchRatingDataTransform
    {
            ...
            CachedTraitCollection traits, ConditionRatingsData ratings)
        {
            ...
            if(!RateConditionType(vector3Conditions, traits, ratings[typeof(UnityEngine.Vector3)]))
                return false;
        ...
        internal static bool RateConditionType(ICondition<UnityEngine.Vector3>[] typeConditions,
            CachedTraitCollection traitCollections, List<Dictionary<int, float>> ratings)
        {
            if (typeConditions.Length == 0)
                return true;

            if (!traitCollections.TryGetType(out List<Dictionary<int, UnityEngine.Vector3>> traits))
                return false;
