...
namespace Unity.Labs.MARS.Data
{
    public partial class MARSDatabase
    {
        bool FindRelationTraits(Relations relations, RelationTraitCache cache)
        {
            ...
            {
                if (!cache.TryGetType(out List<RelationTraitCache.ChildTraits<System.Single>> floatTraits))
                    return false;

                if (!FindTraitCollections(FloatTraitProvider, floatRelations, floatTraits))
                    return false;
            }
            ...
            Dictionary<IMRObject, SetChildArgs> children, HashSet<IMRObject> nonRequiredChildrenLost)
        {
            ...
            if (relations.TryGetType(out IRelation<System.Single>[] floatRelations) && floatRelations.Length > 0)
            {
                if (!CheckIfDataStillMatches(dataAssignments, floatRelations, FloatTraitProvider, 
                    children, nonRequiredChildrenLost))
                    return false;
            }
            ...
            Relations relations, Dictionary<IMRObject, QueryResult> childResults)
        {
            ...
            if (relations.TryGetType(out IRelation<System.Single>[] floatRelations) && floatRelations.Length > 0)
            {
                foreach (var relation in floatRelations)
                {
                    var child1 = relation.child1;
                    if (childResults.TryGetValue(child1, out var child1Result))
                    {
                        var trait1Name = relation.child1TraitName;
                        FloatTraitProvider.TryGetTraitValue(dataAssignments[child1], trait1Name, out var value1);
                        child1Result.SetTrait(trait1Name, value1);
                    }
                    ...
                    {
                        var trait2Name = relation.child2TraitName;
                        FloatTraitProvider.TryGetTraitValue(dataAssignments[child2], trait2Name, out var value2);
                        child2Result.SetTrait(trait2Name, value2);
                    }
                }
            }
