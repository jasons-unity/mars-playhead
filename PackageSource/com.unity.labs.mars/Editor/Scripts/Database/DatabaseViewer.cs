...
namespace Unity.Labs.MARS.Data
{
    class MARSDatabaseViewer : EditorWindow, IHasCustomMenu
    {
        ...
        static GUIStyle s_BiggestFoldoutLabelStyle;
        ...
        void OnGUI()
        {
            ...
            {
                EditorGUILayout.LabelField("compiling, be right back...");
                return;
            }
            ...
            s_ShowDataOwnership = EditorGUILayout.Foldout(s_ShowDataOwnership, "Data Ownership", true, s_BiggestFoldoutLabelStyle);
            ...
            s_ShowProviderData = EditorGUILayout.Foldout(s_ShowProviderData, "Trackable Data", true, s_BiggestFoldoutLabelStyle);
            ...
            s_ShowTraitData = EditorGUILayout.Foldout(s_ShowTraitData, "Trait Data", true, s_BiggestFoldoutLabelStyle);
            if (s_ShowTraitData)
            {
        ...
        {
            s_BiggestFoldoutLabelStyle = new GUIStyle(EditorStyles.foldout);
            s_BiggestFoldoutLabelStyle.fontSize = 14;
            s_BiggestFoldoutLabelStyle.fontStyle = FontStyle.Bold;
        }
        ...
        void DrawDataOwnership(MARSDatabase db)
        {
            using (var scope = new EditorGUILayout.ScrollViewScope(m_DataOwnershipScrollPosition))
            {
                ...
                EditorGUIUtils.DrawDictionaryWithHeader(db.DataUsedByQueries, "Data Used By Queries",
                    "Query ID", "Data IDs Used", m_OwnershipKeyColumnWidth, m_OwnershipValueColumnWidth,
                    CollectionExtensions.Stringify);

                EditorGUILayout.Space();
                EditorGUIUtils.DrawBoxSplitter();
                EditorGUIUtils.DrawDictionaryWithHeader(db.DataUsedByQueryMatches, "Data Used By Query Matches",
                    "Query ID", "Data IDs Used", m_OwnershipKeyColumnWidth, m_OwnershipValueColumnWidth);
        ...
        {
            if (provider.dictionary.Count < 1)
                return;
