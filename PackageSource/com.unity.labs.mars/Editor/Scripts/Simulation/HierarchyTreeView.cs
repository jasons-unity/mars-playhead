...
namespace Unity.Labs.MARS
{
    ...
    public class HierarchyTreeView : TreeView
    {
        class Styles
        {
            ...
            public readonly Color pingHighlightColor;
            ...
            public Styles()
            {
                ...
                pingHighlightColor = MARSUserPreferences.instance.highlightedSimulatedObjectColor;
                defaultRowTextColor = EditorStyles.label.normal.textColor;

                rowLabelStyle = new GUIStyle(GUI.skin.label);

                // The match count will be centered in space for 2 digits
                matchCountWidth = (int)rowLabelStyle.CalcSize(new GUIContent("99")).x;
            }
        }
        ...
        protected override IList<TreeViewItem> BuildRows(TreeViewItem treeRoot)
        {
            ...
            var contentScene = simSceneModule.ContentScene;
            var environmentScene = simSceneModule.EnvironmentScene;
            if (!contentScene.IsValid() || !environmentScene.IsValid())
                return rows;
            ...
            if (!m_IsEnvironmentHierarchy)
            {
                ...
                {
                    contentScene.GetRootGameObjects(m_HierarchyRootGameObjects);
            ...
            {
                environmentScene.GetRootGameObjects(m_HierarchyRootGameObjects);
            ...
            if (m_IsEnvironmentHierarchy && m_HierarchyRootGameObjects.Count > 0)
            {
                var environmentPrefab = m_HierarchyRootGameObjects.FirstOrDefault(
                    f => f.name.Contains(SimulationSettings.environmentPrefab.name));
        ...
        static Color GetTextColorForRow(RowGUIArgs args)
        {
            if (k_PingedIDs.Contains(args.item.id))
                return styles.pingHighlightColor;
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
            ...
            if (isSimEnvRootPrefab)
            {
                if (GUI.Button(infoRect, new GUIContent(MARSEditorGUI.Styles.TabNext), GUIStyle.none))
                    AssetDatabase.OpenAsset(SimulationSettings.environmentPrefab);
            }
        ...
        public Vector2 DrawScrollingTreeView(Vector2 scrollPosition, ref bool scrollToSelected, bool enabled)
        {
            using (var areaScope = new EditorGUILayout.VerticalScope(GUIStyle.none, GUILayout.Height(MaxHeight)))
            {
                    ...
                    GUILayout.Height(MaxHeight)))
                {
                    ...
                    scrollPosition.y = selectedRowY + EditorStyles.foldout.lineHeight;
